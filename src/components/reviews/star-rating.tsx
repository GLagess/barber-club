...
interface StarRatingProps {
  ...
  onChange?: (v: number) => void; // se undefined, é só display
  size?: "sm" | "md" | "lg";
}
    ...
    <div className="flex gap-0.5">
      {[1, 2, 3, 4, 5].map((star) => (
        <button
          ...
          aria-label={interactive ? `${star} estrelas` : undefined}
        >
          <svg
            className={`${cls} transition-colors ${star <= value ? "text-gold" : "text-dark-border"}`}
            fill="currentColor"
            viewBox="0 0 20 20"
          >
