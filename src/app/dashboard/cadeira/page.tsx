...
export default async function DashboardCadeiraPage() {
  ...
  const user = await prisma.user.findUnique({
    ...
    include: {
      barberLinks: { where: { status: "ACCEPTED" }, include: { barbershop: true } },
      reviewsReceived: {
      ...
      <div className="max-w-5xl mx-auto space-y-8">
        <div>
          ...
          <p className="text-muted-foreground">
            {activeLink ? `Vinculado à ${activeLink.barbershop.name}` : "Solicite vínculo com uma barbearia"}
          </p>
        </div>

        {!activeLink ? (
          <div className="rounded-lg border border-dark-border bg-dark-card p-8 text-center space-y-3">
            <p className="text-muted-foreground">Você não está vinculado a nenhuma barbearia ainda.</p>
            <button className="rounded-lg bg-gold px-6 py-2 font-semibold text-background hover:bg-gold-light transition-colors">Buscar Barbearia</button>
          </div>
        ) : (
          <>
